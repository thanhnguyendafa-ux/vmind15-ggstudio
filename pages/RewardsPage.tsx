        ...
        <div className="p-4 sm:p-6 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div>
                    ...
                    <div className="bg-secondary p-6 rounded-lg h-48 flex flex-col items-center justify-center text-center">
                        <LayoutGridIcon className="w-10 h-10 text-slate-500 mb-2"/>
                        <p className="text-text-secondary">Your unlocked badges will appear here.</p>
                 ...
                 <div>
                    ...
                    <div className="bg-secondary p-6 rounded-lg h-48 flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-text-secondary">A log of your XP and badge events.</p>
                        <p className="text-xs text-slate-500">Coming Soon!</p>
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {FIBONACCI_MILESTONES.map(milestone => {
                        const unlocked = userXP >= milestone.xp;
                        return (
                             <div key={milestone.name} className={`p-4 rounded-lg text-center transition-all ${unlocked ? 'bg-yellow-900/50 border-2 border-yellow-600' : 'bg-secondary'}`}>
                                <TrophyIcon className={`mx-auto w-10 h-10 mb-2 transition-colors ${unlocked ? 'text-yellow-400' : 'text-slate-500'}`} />
                                <p className={`font-bold ${unlocked ? 'text-text-primary' : 'text-text-secondary'}`}>{milestone.name}</p>
                                <p className={`text-xs ${unlocked ? 'text-yellow-400' : 'text-slate-500'}`}>{milestone.xp.toLocaleString()} XP</p>
                            </div>
                        )
                    })}
                </div>
