    ...
    return (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" aria-modal="true" role="dialog">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-xl max-w-md w-full max-h-[90vh] flex flex-col">
                <h3 className="text-2xl font-bold text-blue-600 dark:text-sky-400 mb-4">Edit Word</h3>
                
                <form className="overflow-y-auto space-y-4">
                    <div>
                        <label htmlFor="edit-keyword" className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Keyword</label>
                        <input
                            id="edit-keyword"
                            name="keyword"
                            type="text"
                    ...
                    {table.columns.map(col => (
                         <div key={col.name}>
                            <label htmlFor={`edit-${col.name}`} className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">{col.name}</label>
                            <input
                                id={`edit-${col.name}`}
                                name={col.name}
                                type="text"
